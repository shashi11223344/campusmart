...
import { Brain, Zap, Radio, BarChart3 } from 'lucide-react';
...
const DEFAULTS = {
  ...
  heroImage: 'https://images.unsplash.com/photo-1531746790731-6c087fecd65a?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80',
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div ref={cardsRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {cards.map((station, i) => {
              ...
              return (
                <div key={station.title} className="group bg-white border border-slate-200/70 rounded-[2rem] hover:shadow-[0_30px_80px_-20px_rgba(15,23,42,0.28)] transition duration-300 hover:-translate-y-1 flex flex-col shadow-[0_20px_60px_-30px_rgba(15,23,42,0.35)] min-h-[360px] overflow-hidden">
                  <div className="relative overflow-hidden aspect-[4/5]">
                    <img src={station.image} alt={station.title} className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
                  ...
                  <div className="px-6 pb-6 pt-5 flex-grow flex flex-col">
                    <h3 className="text-xl font-semibold text-slate-900 tracking-tight mb-3">{station.title}</h3>
                    <p className="text-sm text-slate-600 leading-relaxed mb-4 flex-grow">{station.description || 'Advanced AI-powered learning solutions.'}</p>
      ...
      <section className="py-16 bg-cm-blue-dark text-white text-center rounded-t-[4rem] border-t-4 border-cm-yellow/50">
        <div className="max-w-4xl mx-auto px-6">
          <h3 className="text-xl md:text-3xl font-bold mb-8 leading-relaxed max-w-2xl mx-auto">
            "AI is not a replacement for education; it's an accelerator. It empowers every student to learn at their own pace and reach their full potential."
          </h3>
          <div className="w-12 h-1 bg-cm-yellow mx-auto mb-4 rounded-full" />
          <div className="font-bold uppercase tracking-widest text-xs text-white/50">Campus Mart AI Learning Philosophy</div>
        </div>
      </section>
    </main>
  );
